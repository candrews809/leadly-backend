...
function renderDashboard(d) {
  const leadsHtml = d.leads.length === 0
    ? \`<div class="empty"><div class="empty-icon">📭</div><p>No leads yet.</p><p style="color:#555;font-size:14px;margin-top:8px">Share your page link to start capturing leads.</p></div>\`
    : d.leads.map(l => {
        const initial = (l.name || '?')[0].toUpperCase();
...
<div class="container">
  ...
  <div class="stats">
    <div class="stat-card"><div class="stat-num">\${d.leadCount}</div><div class="stat-label">Total leads</div></div>
    <div class="stat-card"><div class="stat-num">\${d.leadsThisMonth}</div><div class="stat-label">This month</div></div>
    <div class="stat-card"><div class="stat-num">\${d.cap === 999999 ? '∞' : d.cap}</div><div class="stat-label">Monthly cap</div></div>
  </div>
  ...
  <div class="section">
    <h3>Integrations <span style="color:#555;font-weight:400;font-size:13px">— send leads to your CRM</span></h3>
    <div class="integration-row">
      <div class="int-label">Zapier / CRM webhook<small>Paste your Zapier catch hook or CRM webhook URL</small></div>
      <input type="url" id="webhook-url" placeholder="https://hooks.zapier.com/…" value="\${d.webhookUrl || ''}">
      <button class="save-int" onclick="saveWebhook()">Save</button>
    </div>
  </div>
