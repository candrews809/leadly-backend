...
async function getDb() {
  ...
  await db.collection("events").createIndex({ receivedAt: -1 });
  await db.collection("events").createIndex({ event: 1, utmSource: 1 });
...
window.leadlyTracking = {
  ...
  trackingPixelUrl: '/api/conversion-pixel',
  MAX_STORED: 50,
  ...
  sendPixel: function (data) {
    try {
      ...
      if (navigator.sendBeacon) {
        ok = navigator.sendBeacon(this.trackingPixelUrl, new Blob([payload], { type: 'application/json' }));
      }
      if (!ok) {
        fetch(this.trackingPixelUrl, {
          method: 'POST', keepalive: true,
          headers: { 'Content-Type': 'application/json' },
          body: payload
        }).catch(function () {});
      }
      var events = JSON.parse(localStorage.getItem('leadly_events') || '[]');
      events.push(data);
      if (events.length > this.MAX_STORED) events = events.slice(-this.MAX_STORED);
...
document.addEventListener('submit', function (e) {
  if (e.target.id === 'signup-form' || e.target.classList.contains('leadly-signup')) {
    ...
    if (emailInput) window.leadlyTracking.trackSignup(emailInput.value, 'free');
  }
});
</script>
</body>
...
const server = createServer(async (req, res) => {
  ...
  // Receives tracking beacons from the signup / lead pages.
  if (req.method === "POST" && url === "/api/conversion-pixel") {
    let body = "";
    req.on("data", c => { body += c; if (body.length > 20000) req.destroy(); });
    req.on("end", async () => {
      try {
        const evt = JSON.parse(body || "{}");
        if (!evt.event) { res.writeHead(400); res.end("no event"); return; }
        const database = await getDb();
        await database.collection("events").insertOne({
          event:       String(evt.event).slice(0, 60),
          email:       (evt.email || "").toLowerCase().slice(0, 120),
          plan:        evt.plan || null,
          amount:      typeof evt.amount === "number" ? evt.amount : null,
          campaignId:  (evt.campaignId || "").slice(0, 80),
          utmSource:   (evt.utmSource || "").slice(0, 80),
          utmMedium:   (evt.utmMedium || "").slice(0, 80),
          utmCampaign: (evt.utmCampaign || "").slice(0, 80),
          url:         (evt.url || "").slice(0, 400),
          referrer:    (evt.referrer || "").slice(0, 400),
          stripePaymentId: (evt.stripePaymentId || "").slice(0, 80),
          receivedAt:  new Date()
        });
