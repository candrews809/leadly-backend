...
function hashPassword(password) { return createHmac("sha256", "leadly-secret").update(password).digest("hex"); }
...
async function sendWelcomeEmail(user) {
  await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      html: `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px"><h1 style="font-size:28px;font-weight:800">Welcome to Leadly! 👋</h1><p style="color:#555;font-size:16px;margin:16px 0">Your free lead capture page is live and ready to share.</p><a href="https://leadly-backend-tgbl.onrender.com/dashboard-page" style="display:inline-block;background:#00e87a;color:#080808;padding:14px 28px;border-radius:8px;text-decoration:none;font-weight:700;margin-top:16px">Go to your dashboard →</a><p style="color:#aaa;font-size:12px;margin-top:32px">— Cole at Leadly</p></div>`,
    }),
  });
}
...
nav{padding:20px 24px;border-bottom:1px solid rgba(255,255,255,0.08);display:flex;align-items:center;justify-content:space-between}
...
.badge{display:inline-block;background:rgba(0,232,122,0.1);color:#00e87a;padding:6px 14px;border-radius:100px;font-size:13px;font-weight:600;margin-bottom:20px}
h1{font-family:'Syne',sans-serif;font-size:28px;font-weight:800;margin-bottom:6px;line-height:1.2}
...
input{width:100%;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.12);color:#fff;padding:14px 16px;border-radius:8px;font-size:16px;outline:none;font-family:'DM Sans',sans-serif;transition:border-color .15s}
input:focus{border-color:rgba(0,232,122,0.5)}
input::placeholder{color:#555}
.btn{width:100%;background:#00e87a;color:#000;border:none;padding:16px;border-radius:8px;font-size:16px;font-weight:700;cursor:pointer;margin-top:8px;font-family:'DM Sans',sans-serif;transition:background .15s}
...
.trust{display:flex;gap:16px;margin-top:20px;justify-content:center;flex-wrap:wrap}
.trust-item{display:flex;align-items:center;gap:5px;color:#666;font-size:12px}
...
<nav>
  ...
  <a href="https://useleadly.io" class="back">← Home</a>
</nav>
<div class="main">
  <div class="box">
    <div id="form-section">
      <div class="badge">✨ Free forever — no credit card</div>
      <h1>Start capturing leads for free</h1>
      <p class="sub">Get your lead page in 60 seconds.</p>
      <div class="tabs">
...
if (localStorage.getItem('leadly_token')) { window.location.href = '/dashboard-page'; }
...
async function doSignup() {
  ...
  try {
    const res = await fetch(API + '/signup', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password, plan: urlPlan || 'free' }) });
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Set Up Your Account — Leadly</title>
<link href="https://fonts.googleapis.com/css2?family=Syne:wght@400;700;800&family=DM+Sans:wght@400;500;600&display=swap" rel="stylesheet">
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'DM Sans',sans-serif;background:#080808;color:#f5f5f0;min-height:100vh;display:flex;flex-direction:column}
nav{padding:20px 24px;border-bottom:1px solid rgba(255,255,255,0.08);display:flex;align-items:center}
.logo{font-family:'Syne',sans-serif;font-weight:800;font-size:22px;text-decoration:none;color:#f5f5f0}
.logo span{color:#00e87a}
.main{flex:1;display:flex;align-items:center;justify-content:center;padding:32px 24px}
.box{width:100%;max-width:400px}
.step{color:#00e87a;font-size:13px;font-weight:600;margin-bottom:12px}
...
input:focus{border-color:rgba(0,232,122,0.5)}
input::placeholder{color:#555}
.btn{width:100%;background:#00e87a;color:#000;border:none;padding:16px;border-radius:8px;font-size:16px;font-weight:700;cursor:pointer;margin-top:8px;font-family:'DM Sans',sans-serif}
.btn:hover{background:#00c96a}
.btn:disabled{opacity:0.6;cursor:not-allowed}
.error{color:#ff5555;font-size:13px;margin-bottom:12px;padding:10px 14px;background:rgba(255,85,85,0.1);border-radius:6px;display:none}
.skip{display:block;text-align:center;color:#555;font-size:13px;margin-top:16px;cursor:pointer;background:none;border:none;font-family:'DM Sans',sans-serif}
...
<div class="main">
  <div class="box">
    ...
    <p class="sub">This helps us personalize your lead page.</p>
    <div id="error" class="error"></div>
    <div class="field"><label>Your name</label><input type="text" id="name" placeholder="Jane Smith" autocomplete="name"></div>
    <div class="field"><label>Business name</label><input type="text" id="biz" placeholder="Smith Marketing" autocomplete="organization"></div>
    <button class="btn" id="save-btn" onclick="saveProfile()">Go to my dashboard →</button>
...
async function saveProfile() {
  ...
  const businessName = document.getElementById('biz').value.trim();
  if (!name || !businessName) {
    const el = document.getElementById('error');
    el.textContent = 'Please fill in both fields.';
...
function skip() { window.location.href = '/dashboard-page'; }

document.addEventListener('keydown', e => { if (e.key === 'Enter') saveProfile(); });
</script>
</body>
</html>`;
}
...
.field{margin-bottom:14px}
label{display:block;font-size:13px;color:#888;margin-bottom:5px;font-weight:500}
input{width:100%;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.12);color:#fff;padding:14px 16px;border-radius:8px;font-size:16px;outline:none;font-family:'DM Sans',sans-serif}
input:focus{border-color:rgba(0,232,122,0.5)}
input::placeholder{color:#555}
.btn{width:100%;background:#00e87a;color:#000;border:none;padding:16px;border-radius:8px;font-size:16px;font-weight:700;cursor:pointer;margin-top:8px;font-family:'DM Sans',sans-serif}
.btn:hover{background:#00c96a}
...
<div class="main">
  <div class="box">
    ...
    <div id="reset-form">
      ...
      <div class="field"><label>Confirm password</label><input type="password" id="confirm-pass" placeholder="Repeat your password" autocomplete="new-password"></div>
...
<div class="container">
  <div class="welcome">Welcome back, \${d.name || 'there'} 👋</div>
  <div class="subtitle">\${d.businessName || 'Your business'}<span class="plan-badge">\${(d.plan||'free').toUpperCase()}</span></div>
  <div class="stats">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;align-items:start">
    ...
    <div class="section" style="margin-bottom:0">
      <div class="section-header"><h3>Find new leads</h3></div>
...
function searchLeads(query) {
  ...
  cards.forEach((card, i) => {
    const l = allLeads[i];
    const match = !q || (l.name||'').toLowerCase().includes(q) || (l.email||'').toLowerCase().includes(q) || (l.phone||'').toLowerCase().includes(q) || (l.message||'').toLowerCase().includes(q);
...
async function removeLead(id, idx) {
  try {
    if (id) { await fetch(API + '/leads/' + id, { method: 'DELETE', headers: { Authorization: 'Bearer ' + token } }); }
    ...
    toast('Lead removed');
  } catch(e) { toast('Failed to remove lead'); }
}
...
async function findLeads() {
  ...
  try {
    const res = await fetch(API + '/search-places?q=' + encodeURIComponent(q), { headers: { Authorization: 'Bearer ' + token } });
    const data = await res.json();
    if (!data.results || data.results.length === 0) { box.innerHTML = '<div class="prospect-empty">No results found.</div>'; return; }
  ...
  } catch(e) { box.innerHTML = '<div class="prospect-empty">Search failed.</div>'; }
}
...
async function addProspect(i) {
  ...
  try {
    const addRes = await fetch(API + '/leads', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + token },
      body: JSON.stringify({ name: p.name, email: '', phone: p.phone || '', address: p.address || '', message: 'Added from lead search', businessSlug: localStorage.getItem('leadly_slug') || 'get-leadly', source: 'prospect_search' })
    });
  ...
  } catch(e) { btn.disabled = false; btn.textContent = 'Add lead'; toast('Failed to add lead'); }
}
...
<nav>
  <a href="https://useleadly.io" class="logo">Lead<span>ly</span></a>
  <div class="nav-right"><a href="/dashboard-page" class="back-btn">← Back to dashboard</a></div>
</nav>
<div class="container">
  <h1>Integrations</h1>
  <p class="subtitle">Connect Leadly to your CRM. Every new lead will be sent there automatically.</p>
  <div class="int-section">
    <div class="int-header"><div class="int-icon">SF</div><div><div class="int-title">Salesforce</div><div class="int-desc">Send leads directly to your Salesforce CRM</div></div></div>
    <div class="field"><label>Salesforce Webhook URL</label><input type="url" id="sf-webhook" placeholder="https://hooks.zapier.com/hooks/catch/…"></div>
    <button class="save-btn" onclick="save('salesforce', 'sf-webhook')">Save</button>
    <div class="how-to"><p>1. Go to <a href="https://zapier.com" target="_blank">Zapier</a> and create a new Zap<br>2. Set trigger to "Webhooks by Zapier" → Catch Hook<br>3. Set action to Salesforce → Create Lead<br>4. Copy the webhook URL and paste it above</p></div>
  </div>
  <div class="int-section">
    <div class="int-header"><div class="int-icon">HS</div><div><div class="int-title">HubSpot</div><div class="int-desc">Send leads directly to your HubSpot CRM</div></div></div>
    <div class="field"><label>HubSpot Webhook URL</label><input type="url" id="hs-webhook" placeholder="https://hooks.zapier.com/hooks/catch/…"></div>
    <button class="save-btn" onclick="save('hubspot', 'hs-webhook')">Save</button>
    <div class="how-to"><p>1. Go to <a href="https://zapier.com" target="_blank">Zapier</a> and create a new Zap<br>2. Set trigger to "Webhooks by Zapier" → Catch Hook<br>3. Set action to HubSpot → Create Contact<br>4. Copy the webhook URL and paste it above</p></div>
  </div>
  <div class="int-section">
    <div class="int-header"><div class="int-icon">GHL</div><div><div class="int-title">GoHighLevel</div><div class="int-desc">Send leads directly to your GoHighLevel account</div></div></div>
    <div class="field"><label>GoHighLevel Webhook URL</label><input type="url" id="ghl-webhook" placeholder="https://hooks.zapier.com/hooks/catch/…"></div>
    <button class="save-btn" onclick="save('gohighlevel', 'ghl-webhook')">Save</button>
    <div class="how-to"><p>1. In GoHighLevel go to Settings → Integrations → Webhooks<br>2. Create a new webhook and copy the URL<br>3. Paste it above — no Zapier needed</p></div>
  </div>
  <div class="int-section">
    <div class="int-header"><div class="int-icon">ZP</div><div><div class="int-title">Zapier / Custom Webhook</div><div class="int-desc">Connect to any app via Zapier or a custom webhook</div></div></div>
    <div class="field"><label>Webhook URL</label><input type="url" id="custom-webhook" placeholder="https://hooks.zapier.com/hooks/catch/…"></div>
...
async function save(type, inputId) {
  ...
  await fetch(API + '/settings/webhook', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + token }, body: JSON.stringify({ webhookUrl: url, webhookType: type }) });
  toast('Saved!');
}
async function loadSettings() {
  ...
  const d = await res.json();
  if (d.webhookUrl) { document.getElementById('custom-webhook').value = d.webhookUrl; }
}
loadSettings();
</script>
</body>
...
const server = createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url === "/onboarding-page") {
    res.writeHead(200, { "Content-Type": "text/html" });
    res.end(generateOnboardingPage()); return;
  }
  ...
  if (req.method === "POST" && url === "/signup") {
    ...
    req.on("end", async () => {
      try {
        ...
        if (!email || !password) { res.writeHead(400, { "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "Email and password are required" })); return; }
        ...
        // Generate a temp slug from email prefix
        const emailSlug = generateSlug(email.split("@")[0] + "-" + Math.random().toString(36).slice(2,6));
        const user = { name: "", email: email.toLowerCase(), password: hashPassword(password), token, businessName: "", slug: emailSlug, plan: userPlan, webhookUrl: "", createdAt: new Date() };
        await database.collection("users").insertOne(user);
        await database.collection("businesses").updateOne({ slug: emailSlug }, { $setOnInsert: { businessName: "", slug: emailSlug, email: email.toLowerCase(), description: "", city: "", createdAt: new Date() } }, { upsert: true });
        sendWelcomeEmail(user).catch(console.error);
        fetch("https://api.resend.com/emails", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${RESEND_API_KEY}` }, body: JSON.stringify({ from: "Leadly <onboarding@resend.dev>", to: NOTIFY_EMAIL, subject: `🆕 New signup: ${email}`, html: `<p>${email} just signed up for Leadly.</p>` }) }).catch(console.error);
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ success: true, token, slug: emailSlug }));
      } catch (err) {
  ...
  if (req.method === "POST" && url === "/update-profile") {
    ...
    req.on("end", async () => {
      try {
        ...
        const database = await getDb();
        const newSlug = generateSlug(businessName || user.slug);
        await database.collection("users").updateOne({ token }, { $set: { name: name || "", businessName: businessName || "", slug: newSlug } });
        await database.collection("businesses").updateOne({ slug: user.slug }, { $set: { businessName: businessName || "", slug: newSlug } });
        res.writeHead(200, { "Content-Type": "application/json" });
  ...
  if (req.method === "POST" && url === "/reset-password") {
    ...
    req.on("end", async () => {
      try {
        ...
        if (!user || !user.resetExpires || new Date() > new Date(user.resetExpires)) { res.writeHead(400, { "Content-Type": "application/json" }); res.end(JSON.stringify({ error: "Reset link is invalid or expired." })); return; }
  ...
  if (req.method === "GET" && url === "/search-places") {
    ...
    try {
      ...
      const results = await Promise.all(raw.map(async p => {
        let phone = "";
        try { const dr = await fetch("https://maps.googleapis.com/maps/api/place/details/json?place_id=" + p.place_id + "&fields=formatted_phone_number&key=" + apiKey); const dd = await dr.json(); phone = dd.result?.formatted_phone_number || ""; } catch(e) {}
        return { name: p.name, address: p.formatted_address, phone, placeId: p.place_id, rating: p.rating };
      }));
      res.writeHead(200, { "Content-Type": "application/json" });
