  ...
  if (req.method === "POST" && url === "/signup") {
    ...
    req.on("end", async () => {
      try {
        ...
        fetch("https://api.resend.com/emails", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${RESEND_API_KEY}` }, body: JSON.stringify({ from: "Leadly <onboarding@resend.dev>", to: NOTIFY_EMAIL, subject: `🆕 New signup: ${name} (${email})`, html: `<p><b>${name}</b> (${email}) just signed up for Leadly.<br>Business: ${businessName}<br>Plan: ${userPlan}</p>` }) }).catch(console.error);
